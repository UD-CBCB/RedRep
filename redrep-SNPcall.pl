...
=item GATK (tested with version 2.8.1)
...
my $ver="redrep-SNPcall.pl Ver. 1.6 (9/2/2014 rev)";
die "\n$ver\n\n" if ($version);
...
### MAIN
	...
	{	logentry("REFERENCE DICTIONARY NOT FOUND: BUILDING");
		$sys=cmd("$java /usr/local/picard-tools-1.67/CreateSequenceDictionary.jar R=$refFasta O=$stub2.dict","Building reference dictionary");
	}	
