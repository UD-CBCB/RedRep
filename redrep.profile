...
REDREP_INSTALL=~/00-scripts/redrep/active-git/RedRep

# Location of picard-tools jarfile (jarfile name should be included)
PICARD_JAR_PATH=/usr/local/picard-tools/picard.jar

# Location of GATK jarfile (jarfile name should be included)
...
# Standard PERL modules plus File::Which and Parallel::ForkManager (both available through CPAN) must be installed
...
export PATH=${REDREPBIN}:${FASTQC_PATH}:${CUTADAPT_PATH}:${BWA_PATH}:${SAMTOOLS_PATH}:${CDHIT_PATH}:${USEARCH_PATH}:${PATH}
